...
export async function GET(request: Request) {
  ...
  const performance = searchParams.get("performance")?.trim()
  const lcp = searchParams.get("lcp")?.trim()
  const label = searchParams.get("label")?.trim() ?? "Case study"
  ...
  const metrics = [
    performance ? { label: "Performance", value: performance } : null,
    lcp ? { label: "LCP", value: lcp } : null,
  ].filter(Boolean) as { label: string; value: string }[]

  return new ImageResponse(
    (
      <div tw="flex w-full h-full text-white bg-black p-16">
        ...
        <div tw="flex flex-col flex-1 justify-between">
          <div tw="flex flex-col">
            <p
              tw="mb-6 font-medium tracking-widest uppercase"
              style={{
                fontFamily: "Roboto",
                fontSize: 22,
                color: "#a1a1aa",
            ...
            {tagline ? (
              <p
                tw="mt-5 font-medium leading-snug"
                style={{
                  fontFamily: "Roboto",
                  fontSize: 28,
                  color: "#d4d4d8",
          ...
          {metrics.length > 0 ? (
            <div tw="flex gap-4">
              {metrics.map((metric) => (
                <div
                  key={metric.label}
                  tw="flex flex-col border border-zinc-700 px-5 py-4"
                  style={{ minWidth: 180 }}
                >
                  <span
                    tw="font-medium tracking-wider uppercase"
                    style={{
                      fontFamily: "Roboto",
                      fontSize: 18,
                      color: "#a1a1aa",
